...
The whole RAG pipeline has many configurations and hyperparameters which can be optimized. As evaluation with LLM-as-a-juge is expensive resource-wise, this thesis cannot cover the whole search space. Thus, three separate experiments are done to optimize document preprocessing, query expansion and chunking hyperparameters before the main experiment comparing preprocessor/chunker pairs. These experiments are also only done on a thematically balanced subset of the evaluation dataset containing 15 question-answer pairs.\\
For the numbers of retrieved (top\_k) and reranked (top\_n) documents, there are different strategies. The most simple one is to use the same top\_k and top\_n for all experiments. However, this bears the problem that the length of the context given to the LLM becomes very uncomparable for different Chunkers, as chunk lengths vary massively. These problems can be fixed by setting a fixed token limit, retrieving as many chunks as fit into the limit and then rerank a certain share of those chunks. This token cap can be good for comparison of chunkers, but it can also disadvantage smaller chunk sizes, as these are made for being more flexible. For some queries, less context is needed, so these chunkers have the advantage of being able to potentially provide more precise context with less tokens. This attribute is taken into consideration by using a similarity threshold. Here, there is still a hard top\_n (dependent on chunk size), but there is the option to retrieve less chunks if the cross-encoding score of lower-ranked chunks is below a certain threshold. Theoretically, the last option should work better, as it is more flexible. However, in first trials, it performed worse than just setting a fixed token cap for all tested thresholds (EXPERIMENT No 3?, \parencite{liu2024lost} 1500).\\
\subsubsection{Document preprocessing}
In a first step, the Markdown conversion by Gemini is compared to the conversion by Docling. Then, the better Markdown Processor is chained with the self-written prompt for linearized text and the results are compared to the text linearization from \textcite{min2024exploring}. For both Markdown and for linearized text, the better method of each of the two comparisons is taken in all following experiments. The preprocessors should preserve the structure from the documents, while still not only working for one chunker. Thus, Fixed Paragraph Chunker with size 1 and overlap 0 is used as this chunker pays some attention to document structure, but it is not as specifically tailored to the documents as the table-based methods. As the longer chunks for this method have between 500 and 600 tokens top\_n is set to 3 and top\_k to 9 in order to stay below 1500 tokens for the whole context. Furthermore, all queries can be answered using only one, in rare cases two paragraphs.\\
\subsubsection{Query expansion}
