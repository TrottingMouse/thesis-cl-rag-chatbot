...
An experiment without chunking is run as a baseline for comparison. For this experiment, the SLM is given all three documents processed in linearized form by the optimized preprocessor setup (GeminiMarkdownProcessor+DirectLLMProcessor).
\section{Results}
...
\begin{itemize}
    \item Answer correctness for all preprocessing and chunking strategies between 0.51 and 0.57, all better than no chunking baseline (0.485)
    \item for Markdown: whole table and paragraph with very similar results in all metrics, both with good answer correctness and high context recall, whole table even highest context recall among all nine strategies
    \item for Markdown: split table and character chunking also similar results, low answer correctness and low context recall
    \item best performing chunkers: Character and Max-Min Chunking on linearized text
    \item
\end{itemize}
\begin{table}[htbp]
    \centering
