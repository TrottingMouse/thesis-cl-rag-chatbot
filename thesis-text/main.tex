...
The whole RAG pipeline has many configurations and hyperparameters which can be optimized. As evaluation with LLM-as-a-juge is expensive resource-wise, this thesis cannot cover the whole search space.
...
$$\begin{aligned}
        N_{\text{top}} & = \max\left(1, \left\lfloor \frac{2000}{S_{\text{avg}}} \right\rfloor\right) \\[6pt]
        K_{\text{top}} & = 3 N_{\text{top}}
    \end{aligned}$$

The token limit was set to 2000??? because (as mentioned in the introduction) performance of SLMs around 8B parameters already drops between 4000 and 8000 input tokens of context. Thus, for the 2B model, the context input was set to less than half.\\
