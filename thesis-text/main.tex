...
\subsection{Comparison of Markdown and Text}
Overall, RAG on linearized text files performed slightly better than on Markdown files. This is probably due to the fact that most of the training data for a language model is plain text. Another metric supporting this hypothesis is faithfulness. Plain text makes it easier for the SLM to adhere to the documents, thus achieving a higher faithfulness.
\subsection{Markdown Chunking Strategies}
\subsubsection{Paragraph Chunking and Whole Table Chunking}
\begin{itemize}
    \item Whole Table and Paragraph for Markdown with similar results because similar strategies: paragraph splitting for markdown often splits at table/module borders and wholeparagraph splits at paragraph level for text documents[see example]
    \item both good at context recall because just has to retrieve the one or two right tables/paragraphs
    \item better than character chunking and SplitTable because keep tables together, LLMs often partly trained on markdown data and can handle as trained, worse if split inside the tables, also harder for the retriever to find right chunks as relevant table is split into several chunks hard to find in structured table format, resulting in low context recall
\end{itemize}
...
\end{itemize}
\subsection{Text Chunking Strategies}
...
Character Chunking has the known problem of cutting off sentences and even words. The loss of context is mitigated by using an overlap. However, these random cuts in the beginning and in the end of a chunk result in worse vector representations and worse retrieval, thus worsening the context recall.
Having said that, Character Chunking still achieves the highest answer correctness. The shorter chunks are easier for the SLM to handle and find the information, resulting in the high faithfulness. Furthermore, the SLM can easily separate the chunks from each other in its context due to their in-word separation.
\begin{itemize}
    \item surprising, normally harder for lm to handle because not normal sentences. assumption: (oben)
    \item            chunk für kann ich praktikum machen wann ich will: "e Kenntnisse zum wissenschaftlichen Arbeiten, können eigene Forschungsfragen formulieren, diese angemessen eingrenzen und die zur Beantwortung notwendigen Schritte planen und umsetzen. Teilnahmevoraussetzungen für das Modul Praktikum \& Orientierung sind im 4. Fachsemester oder höher, und inhaltlich das Basismodul - Mathematische und logische Grundlagen, das Basismodul - Grundlagen der Computerlinguistik und formale Grammatikbeschreibung und das Basismodul - Python-Programmierung. Für dieses Modu"  (ende des chunks, anfang der frage)
    \item also problems with not finding information in context but less than others
\end{itemize}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{l X X}
        ...
        \bottomrule
    \end{tabularx}
    \caption{Comparison of Successful Extraction versus Missing Context Retrieval for Character Chunking}
    \label{tab:slm_evaluation_comparison_retrieval}
\end{table}
