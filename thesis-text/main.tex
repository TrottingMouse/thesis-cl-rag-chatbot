...
Thus, four separate experiments are done to optimize document preprocessing, context size, query expansion and chunking hyperparameters before the main experiment, which is comparing preprocessor/chunker pairs.
These experiments are also only done on a thematically balanced subset of the evaluation dataset containing 15 question-answer pairs.\\
...
\subsubsection{Query expansion}
Using the results from the previous two experiments, the two query expansion methods and no query expansion are compared. As the influence of chunk size is not as big as for the context size experiment, only Paragraph Chunking was chosen here, as in the preprocessing experiment.\\
...
For Max-Min Chunking, the hard baseline threshold (hard\_threshold) and the dampening parameter (c) are tuned in a grid search. As the documents are thematically very homomgenous, values for both hard\_threshold and c are chosen higher or equal than the default values.
