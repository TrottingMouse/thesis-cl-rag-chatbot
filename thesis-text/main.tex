...
In the end, the best chunkers used plain text documents, but the best chunkers using Markdown documents were not considerably behind. A key distinction here is that for plain text, chunks should be smaller, while for Markdown, table integrity is more important.\\
Regarding chunking, a crucial insight emerged: more complex frameworks (such as LumberChunker or MiniRAG) failed on this specific task, and even successful semantic methods like Max-Min chunking required significant hyperparameter adaptation. This demonstrates that complex RAG methods cannot be applied out-of-the-box to small, homogeneous, administrative corpora. Instead, deterministic or highly tailored chunkers are more suitable. Character Chunking, as a highly deterministic method, surprisingly, achieved the highest answer correctness overall.\\
Ultimately, the most effective pipeline configuration identified relies on LLM-based linearization, Max-Min semantic chunking, and a conservative cross-encoder threshold of 0.05, offering an optimal setup for resource-constrained environments.
\begin{figure}[htbp]
    \centering
...
The questions were conciously formulated in a short and colloquial manner in order to simulate realistic user interactions. This sometimes runs contrary to the requirement of unambiguity, but in the context of the documents, both requirements were met in the best possible way.
...
The first one, Whole Table Paragraph Chunker, splits the structured Markdown documents into entire tables (for fPO) or modules (for MH). Each table chunk receives the abbreviation legend and its heading as contextualization. The normal, unstructured text in the Markdown documents is split into paragraphs.\\
...
An important attribute of RAG pipelines which cannot be captured by the previous four metrics is negative rejection. This is the ability to reject questions which cannot be answered with the given context. Negative rejection is important, as a hallucinated answer for an administrative question can have severe consequences for students. For the RAGAS framework, a metric with a custom prompt was implemented to measure negative rejection (see Listing \ref{lst:neg_rejection_prompt}). This metric was used for the second dataset containing unanswerable queries.\\
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{} X X @{}}
        \toprule
        \multicolumn{2}{@{}p{\textwidth}@{}}{\textbf{Splittable} \textit{\footnotesize(Bad Grammar)} \hfill \textbf{Query:} Kann ich Einführung in die Programmierung mit Python auch im zweiten Semester belegen?}                                                                                                                          \\
        \midrule
        ...
        \midrule[\heavyrulewidth]
        \multicolumn{2}{@{}p{\textwidth}@{}}{\textbf{Splittable} \textit{\footnotesize(Missed Intent)} \hfill \textbf{Query:} Gibt es in Machine Learning eine Klausur?}                                                                                                                                                                     \\
        \midrule
        \textbf{Reference} & \textbf{Model Response}                                                                                                                                                                                                                                                                                         \\
        \cmidrule(lr){1-1} \cmidrule(lr){2-2}
...
\begin{longtable}{
    ...
    \caption[]{(Fortsetzung) Evaluation Analysis}                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \\
    \toprule
    \textbf{\#}                                                                                                                                                                                                                                                                                                                                                                                                                              & \textbf{Query} & \textbf{Response} & \textbf{Reference Answer} & \textbf{Correctness} \\
    \midrule
    \endhead
    ...
    \midrule
    \multicolumn{5}{r}{\footnotesize\textit{Fortsetzung auf der nächsten Seite\dots}}                                                                                                                                                                                                                                                                                                                                                                                                                                                \\
    \endfoot
...
Future research could investigate all of the suggested improvements.\\
