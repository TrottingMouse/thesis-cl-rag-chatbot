...
Concerning evaluation, more frameworks could be tested with even more robustness to differences in formulation.



\section{Conclusion}
This thesis investigated the optimization of Retrieval-Augmented Generation (RAG) pipelines for Small Language Models (SLMs), using structured administrative documents from the Department of Computational Linguistics to develop a local, resource-efficient student advisory assistant. Because parameter-constrained models like Qwen3.5-2B are highly susceptible to lost-in-the-middle phenomena when processing dense contexts, the offline and online components of the pipeline were systematically evaluated to maximize generation quality and factual reliability, focusing on chunking, document preprocessing and query expansion.

For evaluation, a dataset of 73 question-answer pairs of varying difficulty was created. These could be answered with the documents and served to evaluate answer correctness. Another 14 questions were created which could not be answered with the documents to test the negative rejection capability of the pipeline.

The empirical results validate the fundamental necessity of targeted document processing and chunking. Chunking combined with preprocessing substantially outperformed unchunked baselines, elevating answer correctness from $0.423$ on raw text to over $0.570$ on optimized configurations. Furthermore, even the worst-preforming chunkers outperformed the unchunked baseline.
...
\begin{itemize}
    \item small chunks for plain text, larger chunks for markdown (not destroy structure)
    \item small chunks need overlap to limit losses for context problem (short explanation)
    \item in-sentence/word separation of character chunking caused less problems for qa-pairs than for previous research (with overlap best performing strategy in terms of answer correctness), however problem with negative rejection
    \item best allround strategy: Max-Min chunking, hyperparameters fitted to homogenous documents with second best answer correctness and good negative rejection
    \item dynamic token chunking could be good alternative with smaller chunks
    \item for Markdown: only strategies leaving tables and modules together prevailed: simple Paragraph Chunker and Whole Table Chunker
    \item best chunkers on linearized text slightly better than best on markdown, but margin not high enough to discard Markdown in the future
    \item document preprocessing: both for Markdown conversion and linearization prompting LLMs with task better results than specifically designed frameworks
    \item reranking threshold tested (give short explanation of concept here): chunkers with small size chunks profit because more flexibility (less noise for queries where clear right chunk), for larger chunks mostly result degradation. however, also not more efficient due to reranking quality: relevant chunks often not retrieved in beginning
    \item Chain of Thought query processing with better performance than HyDE, however no query expansion also with good results (improved embedding model architecture for RAG)
    \item effectiveness of query preprocessing shown: even without chunking better performance than with raw documents
    \item all chunking methods clearly better AC than preprocessed and no chunking
\end{itemize}
